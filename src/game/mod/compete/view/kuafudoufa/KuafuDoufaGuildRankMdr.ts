namespace game.mod.compete {
    ...
    import DoufaXianzongPaimingConfig = game.config.DoufaXianzongPaimingConfig;

    export class KuafuDoufaGuildRankMdr extends EffectMdrBase {
        private _view: RankView = this.mark("_view",RankView);

        private _itemList: ArrayCollection;
        private _proxy: CompeteProxy;
        private _type: number = KuafuDoufaOpType.GuildRank;
        ...
        private initShow(): void {
            ...
            this._proxy.c2s_kuafudoufa_click(KuafuDoufaOpType.GuildRank);
        }

        private updateShow(): void {
            let cfg: ParamConfig = GameConfig.getParamConfigById("kuafu_doufa_xianzong_paiming");
            let limit = cfg && cfg.value;
            let rankStr = StringUtil.substitute(getLanById(LanDef.kuafu_doufa_tips13), [StringUtil.getHurtNumStr(limit)]);
            ...
            this._view.lab_num.text = getLanById(LanDef.kuafu_doufa_tips15) + "：" + StringUtil.getHurtNumStr(count);//宗门伤害：0
            ...
            for(let i = 0; i < MAX_RANK_NUM; ++i){
                ...
                if(rankInfo){
                    name = rankInfo.guild_name + "\n" + getLanById(LanDef.cross_boss_tips10) + "：" + rankInfo.name;
        ...
        private getReward(rank: number): number[][] {
            let cfgList: DoufaXianzongPaimingConfig[] = getConfigListByName(ConfigName.DoufaXianzongPaiming);
